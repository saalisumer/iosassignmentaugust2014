...
static MBProgressHUD *mLastVisibleInstance;
...
+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    ...
    if (!view && mLastVisibleInstance) {
        [mLastVisibleInstance hide:NO];
    }
	return [huds count];
}
...
+ (MBProgressHUD *)lastVisibleHUD {
	return mLastVisibleInstance;
}
...
- (void)show:(BOOL)animated {
    ...
    if (mLastVisibleInstance) {
        mLastVisibleInstance.alpha = 0.0;
        [mLastVisibleInstance done];
    }
	mLastVisibleInstance = self;
}
...
- (void)hide:(BOOL)animated {
    ...
    if ([self isEqual: mLastVisibleInstance]) {
        mLastVisibleInstance = nil;
    }
}
...
- (void)updateIndicators {
	...
	if (mode == MBProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		...
		self.indicator = MB_AUTORELEASE([[UIActivityIndicatorView alloc]
										 initWithActivityIndicatorStyle:activityIndicatorStyle]);
